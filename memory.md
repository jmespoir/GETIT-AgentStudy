...
냉장고에 있는 재료를 등록하면 만들 수 있는 레시피를 추천해 주는 웹 서비스.
스택(CLAUDE.md 고정): React(Vite) / Express.js / Supabase / Vercel, JavaScript.
...
브랜치 `main`, origin/main과 동기화됨. 미커밋 변경: `promptLog.md` 1건 (수정).
...
코드·문서 전체에서 검색 결과 **없음** (js, jsx, sql, md, html 대상).

## 확인 필요

- `server/db/schema.sql`을 Supabase에서 실제로 실행했는지 (테이블·RLS 적용 여부)
- Supabase 대시보드 설정 — 이메일 로그인 활성화, Site URL, 매직링크 템플릿의 `{{ .Token }}` 추가 여부
- 인증된 재료 CRUD의 실제 응답 (실행 미검증)
