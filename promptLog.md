...
**프롬프트 원문**
```
memory.md를 읽고 무엇일 시작해야할지 알려줘
```

**결정사항**
- `schema.sql` 실행·토큰 발급은 사용자만 할 수 있는 작업이라, 에이전트는 그와 무관하게 진행 가능한 즐겨찾기 API부터 착수
- 추천 API는 레시피 데이터 출처 미정으로 계속 보류
...
**결정사항**
- 중복 즐겨찾기는 재료 API와 동일하게 **409**로 응답 (멱등 200 대신 일관성 우선)
- 해제는 행 `id`가 아니라 `recipe_id`로 삭제 — 레시피 카드에서 바로 해제할 수 있어야 하므로
- `recipe_id`는 문자열로만 받고 공백 트림. 빈 값·미전달·비문자열은 400
- 새 패키지·새 환경 변수·스키마 변경 없음 (`favorites` 테이블은 기존 `schema.sql`에 이미 정의돼 있었음)

---

### 15. access token으로 실동작 검증

**프롬프트 원문**
```
<access token 값 — 비밀이라 기록하지 않음>
토큰 값 바탕으로 검증을 진행해줘
```

**결정사항**
- 토큰은 스크래치패드에만 두고 검증 후 삭제, 프로젝트 파일·로그에는 남기지 않음
- 포트 4000에 이전 세션의 옛 서버가 떠 있어, 사용자 프로세스를 건드리지 않고 포트 4100에서 검증
- 즐겨찾기 12개 케이스, 재료 전 경로 통과. Supabase 테이블·RLS가 이미 적용돼 있음을 확인
- 검증 중 `DELETE /api/ingredients/:id`에 uuid가 아닌 값이 오면 500이 나는 기존 버그 발견
- 테스트로 만든 데이터는 전부 삭제, 기존 데이터(`계란`)는 건드리지 않음
- RLS 격리는 계정이 하나뿐이라 미검증으로 남김

---

### 16. uuid 500 수정 후 커밋·푸시

**프롬프트 원문**
```
uuid 500 먼저 고치고 같이 커밋해줘
```
```
커밋하고 push까지 해줘
```
...
**결정사항**
- `error.code === '22P02'`를 **404**로 분기 (400 대신) — uuid 형식이 아닌 id는 어떤 행과도 매칭될 수 없어 '없는 재료'와 결과가 같고, 클라이언트 에러 처리가 한 갈래로 끝남
- 커밋은 두 개로 분리 (서버 코드 / 메모리 문서)
- `main`에 커밋 후 push (`2619130`, `5db17ae`)
...
**프롬프트 원문**
```
promptLog.md를 갱신하고 깃허브에 올려줘
```

**결정사항**
- 프롬프트에 포함된 access token은 기록하지 않고 자리 표시만 남김
- 커밋 `3bc2cc7`~`e11714e`(메모리 문서 작업)의 프롬프트 원문은 남아 있지 않아 임의로 복원하지 않음
