...
- **인증된 재료 CRUD 실동작** — 토큰이 있는 상태의 조회/추가/삭제(200·201·404·409)는 한 번도 실행해 보지 못했다. 확인하려면 Supabase 테이블 생성과 실제 access token이 필요하다.
- 확인된 것은 인증 실패 경로뿐: health 200, 토큰 없음/잘못된 토큰 401, 없는 경로 404, 깨진 JSON 400 (2026-09-21 로컬 실행).
