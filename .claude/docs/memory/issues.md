...
- **에이전트 팀 실제 동작** — 에이전트 파일 구조, 차단 훅, 스킬 이름만 검증했다. 새 세션에서 에이전트가 목록에 뜨는지, 실제 위임이 되는지는 미확인. 특히 `/gstack-design-review`는 커밋 단계가 차단되고 깨끗한 작업 트리 요구도 못 맞추는 상태에서 어떻게 진행할지 미검증
